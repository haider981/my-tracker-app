...
const approveEntryRequest = async (req, res) => {
    try {
        ...
        // ✅ QUEUE NOTIFICATIONS - FIXED
        try {
            if (employee) {
                // 1. Notify the employee
                await queueNotification({
                    userId: employee.id,
                    type: 'MISSING_ENTRY_APPROVED_BY_ADMIN',
                    data: {
                ...
                console.log(`📮 Queued employee notification for ${employee.name}`);

                // 2. Notify the SPOC (if exists)
                if (employee.spoc_email && employee.spoc_name) {
                    const spoc = await prisma.users.findFirst({
                        where: { 
                            email: employee.spoc_email,
                            role: { in: ['SPOC', 'spoc', 'Spoc'] }
                        },
                    ...
                    if (spoc) {
                        await queueNotification({
                            userId: spoc.id,
                            type: 'SPOC_MISSING_ENTRY_APPROVED',
                            data: {
                                spocId: spoc.id,
                                adminName: adminName,
                        ...
                        console.log(`📮 Queued SPOC notification for ${spoc.name}`);
                    }
                }
            }
        } catch (notifError) {
            console.error("[approveEntryRequest] Notification queue error:", notifError);
    ...
    try {
        ...
        try {
            if (employee) {
                ...
                await queueNotification({
                    userId: employee.id,
                    type: 'MISSING_ENTRY_REJECTED_BY_ADMIN',
                    data: {
                        employeeId: employee.id,
                        adminName: adminName,
                        entryDate: existingWorklog.date,
                ...
                if (employee.spoc_email && employee.spoc_name) {
                    ...
                    if (spoc) {
                        await queueNotification({
                            userId: spoc.id,
                            type: 'SPOC_MISSING_ENTRY_REJECTED',
                            data: {
                                spocId: spoc.id,
                                adminName: adminName,
                                employeeName: employee.name,
            ...
            console.error("[rejectEntryRequest] Notification queue error:", notifError);
            // Don't fail the request if notification fails
        }
...
const bulkApproveEntryRequests = async (req, res) => {
    try {
        ...
        // Get all unique employee names and fetch their IDs + SPOC info
        const uniqueEmployeeNames = [...new Set(eligibleWorklogs.map(w => w.name))];
        const employees = await prisma.users.findMany({
            where: { name: { in: uniqueEmployeeNames } },
            select: { 
                id: true, 
                name: true, 
                email: true,
        ...
        const approvedEntries = [];
        const failedEntries = [];
        const spocNotificationsMap = new Map(); // Track SPOC notifications
        ...
        for (const worklog of eligibleWorklogs) {
            try {
                ...
                // ✅ QUEUE NOTIFICATIONS - FIXED
                try {
                    const employee = employeeMap.get(worklog.name);
                    if (employee) {
                        // 1. Notify the employee
                        await queueNotification({
                            userId: employee.id,
                        ...
                        // 2. Track SPOC notifications (send once per SPOC at the end)
                        if (employee.spoc_email && employee.spoc_name) {
                            if (!spocNotificationsMap.has(employee.spoc_email)) {
                                spocNotificationsMap.set(employee.spoc_email, {
                                    spocEmail: employee.spoc_email,
                                    spocName: employee.spoc_name,
                                    employees: []
                                });
                            }
                            spocNotificationsMap.get(employee.spoc_email).employees.push({
                                name: employee.name,
                                entryDate: worklog.date
                            });
                        }
                    ...
                    console.error(`[bulkApproveEntryRequests] Notification queue error for worklog ${worklog.id}:`, notifError);
                    // Don't fail the request if notification fails
                }
            } catch (error) {
                console.error(`Error processing worklog ${worklog.id}:`, error);
        ...
        // Send SPOC notifications (once per SPOC)
        for (const [spocEmail, spocData] of spocNotificationsMap.entries()) {
            try {
                const spoc = await prisma.users.findFirst({
                    where: { 
                        email: spocEmail,
                ...
                if (spoc) {
                    await queueNotification({
                        ...
                        data: {
                            spocId: spoc.id,
                            adminName: adminName,
                            employeeCount: spocData.employees.length,
                            employeeNames: spocData.employees.map(e => e.name).join(', '),
                            entryDate: spocData.employees[0].entryDate,
                            requestId: null
                        }
                    });
                    console.log(`📮 Queued SPOC notification for ${spoc.name} (${spocData.employees.length} employees)`);
                }
            } catch (notifError) {
                console.error(`[bulkApproveEntryRequests] SPOC notification error for ${spocEmail}:`, notifError);
            }
        }
...
const getPendingEntryRequestsCount = async (req, res) => {
    try {
        ...
        console.error('Error fetching pending entry requests count:', error);
        res.status(500).json({
            success: false,
            message: 'Failed to fetch pending entry requests count',
            error: error.message
        });
    }
};
