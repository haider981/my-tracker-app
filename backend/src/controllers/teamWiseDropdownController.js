...
// ===============================
// GET: Global search across all teams
// ===============================
const globalSearchDropdownValues = async (req, res) => {
  try {
    const { query, type, excludeTeam, exact } = req.query;
    ...
    if (type && validHeaders.includes(type))
      where.column_header = type;

    if (excludeTeam)
      where.team = { not: excludeTeam };

    const results = await prisma.teamWiseDropdowns.findMany({
      where,
      select: { values: true, column_header: true, team: true },
      orderBy: [{ team: 'asc' }, { column_header: 'asc' }, { values: 'asc' }],
    });

    const TYPE_LABELS = {
      book_element: 'Book Element',
    ...
    const data = results.map((r) => ({
      value: r.values,
      type: r.column_header,
      typeLabel: TYPE_LABELS[r.column_header] || r.column_header,
      team: r.team,
    }));
    ...
    res.status(500).json({ success: false, message: 'Failed to search dropdown values', error: error.message });
  }
};
