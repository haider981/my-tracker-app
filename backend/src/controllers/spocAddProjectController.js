...
const updateProjectStatus = async (req, res) => {
    try {
        ...
        const validStatuses = ['In Review', 'Approved', 'Rejected', 'Hide Requested', 'Hidden', 'Hide-Request-rejected', 'Request-Unhide'];
...
// SPOC can request to hide an existing project.
const requestHideProject = async (req, res) => {
    try {
        const { project_id } = req.params;
        const { email } = req.user || {};
        ...
        if (!project) {
            return res.status(404).json({
                success: false,
                message: "Project not found for current SPOC",
            });
        }

        if (project.audit_status === "Hidden") {
            return res.status(400).json({
                success: false,
                message: "Project is already hidden",
            });
        }

        if (project.audit_status === "Hide Requested") {
            return res.status(409).json({
                success: false,
                message: "Hide request is already pending for this project",
            });
        }

        const updatedProject = await prisma.projectRecords.update({
            where: { project_id },
            data: { audit_status: "Hide Requested" },
        });

        res.status(200).json({
            success: true,
            message: "Hide project request sent to admin",
            data: {
                projectId: updatedProject.project_id,
                projectName: updatedProject.project_name,
                status: updatedProject.audit_status,
        ...
        console.error("Error requesting hide project:", error);
        res.status(500).json({
            success: false,
            message: "Internal server error",
            error: error.message,
...
// SPOC can request to unhide a hidden project.
const requestUnhideProject = async (req, res) => {
    try {
        const { project_id } = req.params;
        const { email } = req.user || {};
        ...
        if (project.audit_status !== "Hidden") {
            return res.status(400).json({
                success: false,
                message: "Only hidden projects can be requested for unhide",
            });
        }

        const updatedProject = await prisma.projectRecords.update({
            where: { project_id },
            data: { audit_status: "Request-Unhide" },
        });

        res.status(200).json({
            success: true,
            message: "Unhide project request sent to admin",
            data: {
                projectId: updatedProject.project_id,
                projectName: updatedProject.project_name,
                status: updatedProject.audit_status,
        ...
        console.error("Error requesting unhide project:", error);
        res.status(500).json({
            success: false,
            message: "Internal server error",
            error: error.message,
