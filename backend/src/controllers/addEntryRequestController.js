...
// Get monthly request statistics for the current user
...
const submitEntryRequest = async (req, res) => {
  try {
    ...
    // Check if this is a new date that would exceed the limit
    const entryDateStr = new Date(entryDate).toISOString().split('T')[0];
    const isNewDate = !uniqueDays.has(entryDateStr);
    
    if (isNewDate && currentMonthDays >= monthlyLimit) {
      return res.status(400).json({
        success: false,
        message: `You have reached your monthly limit of ${monthlyLimit} days for missing entry requests. You cannot submit more requests this month.`
      });
    }
    ...
    // Check for duplicate entry (same date, project, task, book element, etc.)
    const existingEntry = await prisma.todaysWorklog.findFirst({
      where: {
        name: userName,
        date: new Date(entryDate),
        project_name: projectId || projectName,
    ...
    if (existingEntry) {
      return res.status(400).json({
        success: false,
        message: 'A similar entry request already exists in your pending requests. Please modify the entry or remove the duplicate.'
      });
    }
