    ...
    try {
      ...
      if (data.success && data.found) {
        if (data.isNA) {
          ...
          // Auto-set units count to 1 and lock if unit type is "general"
  ...
  const fetchPastRequests = useCallback(async () => {
    ...
    try {
      ...
      if (data?.success && Array.isArray(data.requests)) {
        ...
        // Extract recent projects for autocomplete - NEW FUNCTIONALITY
...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    // Show recent projects when input is empty - NEW FUNCTIONALITY
    if (!q) {
  ...
  const selectProject = (p) => {
    ...
    if (p.dueOn) {
      setDueOn(p.dueOn);
      setIsDueDateLocked(true); // Lock due date when project is selected - NEW FUNCTIONALITY
    }
    setShowSuggest(false);
    setSelectedSuggestionIndex(-1);
  };
  ...
  const clearForm = () => {
    ...
    setIsDueDateLocked(false); // Unlock due date on clear - NEW
  };
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (data?.success) {
        ...
        clearForm();
        await fetchPendingRequests();
        await fetchMonthlyRequestStats();
      } else {
        setSubmitMsg("Failed to submit request. Please try again.");
      }
    } catch (error) {
  ...
  // Check if units count should be locked (when unit type is "general")
  const isUnitsCountLocked = unitsType === "general";
        ...
        <main className="flex-1 transition-all duration-300 ease-in-out lg:ml-72 overflow-y-auto h-[calc(100vh-4rem)]">
          <div className="w-full px-4 sm:px-6 lg:px-8 py-8">
            ...
            <form onSubmit={onSubmit} className="bg-white rounded-2xl shadow-xl p-4 sm:p-6 border border-slate-200">
              ...
              {submitMsg && (
                <div className={`mb-4 rounded-xl border-2 px-4 py-3 text-sm flex items-start shadow-md ${submitMsg.includes("success") || submitMsg.includes("Successfully")
              ...
              <div className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 ${monthlyRequestStats.isLimitReached ? 'opacity-50 pointer-events-none' : ''}`}>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          if (newValue !== projectId) {
                            setProjectId("");
                            setProjectName("");
                            setIsDueDateLocked(false); // Unlock when clearing project
                          }
                          if (!newValue.trim()) {
                            setDueOn("");
                ...
                <Field label="No. of Units *">
                  <div className="flex gap-2 items-start">
                    <input
                      type="number"
                      className={`flex-1 h-9 text-sm px-3 rounded-2xl border-2 ${invalid.unitsCount ? "border-red-500" : "border-slate-300"
                        } focus:border-indigo-600 ${isUnitsCountLocked ? "bg-slate-100 cursor-not-allowed" : ""}`}
                      placeholder="e.g., 10"
                      value={unitsCount}
                      onChange={(e) => {
                        if (!isUnitsCountLocked) {
                      ...
                      disabled={monthlyRequestStats.isLimitReached || isUnitsCountLocked}
                      readOnly={isUnitsCountLocked}
                    />
                ...
                <Field label="Due On">
                  <input
                    type="date"
                    className={`w-full h-9 text-sm px-3 rounded-2xl border-2 border-slate-300 focus:border-indigo-600 ${isDueDateLocked ? "bg-slate-100 cursor-not-allowed" : ""}`}
                    value={dueOn}
                    onChange={(e) => {
                      if (!isDueDateLocked) {
                    ...
                    disabled={monthlyRequestStats.isLimitReached || isDueDateLocked}
                    readOnly={isDueDateLocked}
                  />
                  {dueOn && (
                    <div className="mt-1 text-xs text-slate-600">
                      Due: {new Date(dueOn).toLocaleDateString()}
                      {isDueDateLocked && <span className="ml-2 text-blue-600"></span>}
                    </div>
                  )}
                </Field>
...
function MultiSelectChips({ value = [], onChange, options = [], placeholder = "Select one or more…", isInvalid = false, bookElement = "", disabled = false }) {
  ...
  const filtered = useMemo(() => {
    ...
    if (bookElement === "Full book" || bookElement === "Full Book") {
        ...
        >
          {filtered.length === 0 ? (
            <div className="px-3 py-2 text-sm text-slate-500">
              {bookElement === "Full book" || bookElement === "Full Book" ? "Only non-numeric chapters allowed for Full book" : "No matches"}
            </div>
          ) : (
            filtered.map((opt) => (
              <div
...
export { SidebarLinks, Field, Select, MultiSelectChips, RequestsBlock, Feedback };
